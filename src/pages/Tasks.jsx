...
export default function Tasks() {
  ...
  const applyFilters = (filters) => {
    ...
    if (filters.status.length)
      updated = updated.filter((t) => filters.status.includes(t.status));
    if (filters.priority.length)
      updated = updated.filter((t) => filters.priority.includes(t.priority));
    if (filters.category)
      updated = updated.filter((t) => t.category === filters.category);
    if (filters.date)
      updated = updated.filter(
        (t) =>
          new Date(t.deadline).toISOString().split("T")[0] === filters.date,
      );
    if (filters.assignedTo)
      updated = updated.filter((t) =>
        t.assignedTo?.toLowerCase().includes(filters.assignedTo.toLowerCase()),
      );
    if (filters.keyword)
      updated = updated.filter((t) =>
        ...
        <div className="divide-y divide-slate-200 rounded-lg bg-white shadow-sm dark:divide-slate-700 dark:bg-[#171717]">
          ...
          {filteredTasks.map((task) => (
            ...
            >
              <div className="flex justify-center">
                <button onClick={() => toggleTaskSelection(task.id)}>
                  {selectedTasks.has(task.id) ? (
                    <SquareCheckBig className="text-green-500" size={20} />
                  ) : (
              ...
              <div>
                <span
                  className={`inline-flex items-center gap-2 rounded-full px-3 py-1 font-medium ${statusConfig[task.status].color}`}
                >
